...
describe('toPassTraceEval', () => {
  it('passes when all expected tools match', () => {
    ...
    expect(spans).toPassTraceEval([{ toolName: 'search', args: { q: 'test' } }, { toolName: 'read_file' }]);
  });
  ...
  it('supports custom pass threshold', () => {
    ...
    expect(spans).toPassTraceEval([{ toolName: 'search' }, { toolName: 'write_file' }], { passThreshold: 0.5 });
  });
  ...
  it('logs warning when score is between warn and pass thresholds', () => {
    ...
    expect(() => {
      expect(spans).toPassTraceEval([{ toolName: 'search' }, { toolName: 'write_file' }], { warnThreshold: 0.4, passThreshold: 0.8 });
    }).toThrow();
  ...
  it('works with .not negation', () => {
    ...
    expect(spans).not.toPassTraceEval([{ toolName: 'search' }, { toolName: 'write_file' }]);
  });
});
